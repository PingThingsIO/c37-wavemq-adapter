			...
			for df := range ch {
				...
				if len(batch) == BatchSize {
					...
					for pi, pn := range batch[0].Data[0].PHASOR_NAMES {
						if include[pn] {
							...
							for i := 0; i < len(batch); i++ {
								ch.Data = append(ch.Data, &c37pb.Phasor{
					...
					for ai, an := range batch[0].Data[0].ANALOG_NAMES {
						if include[an] {
							ch := &c37pb.ScalarChannel{
								ChannelName: an,
								Unit:        "Analog",
							}
							for i := 0; i < len(batch); i++ {
								ch.Data = append(ch.Data, &c37pb.Scalar{
									Time:  batch[i].UTCUnixNanos,
